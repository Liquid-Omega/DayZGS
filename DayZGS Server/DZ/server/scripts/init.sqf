diag_log "SERVER: START server\init.";
...
player_harvestPlant	=		compile preprocessFileLineNumbers "\dz\server\scripts\players\player_harvestPlant.sqf";
...
// admin functions ****************************************************************************************************
	...
	//[_id,_uid,_agent] ExecVM "\DZ\_admin\admin_check.sqf";
...
fnc_processItemWetness = {
	...
	*/
	...
	if (isNull _item)exitWith{
		_playerWet = (((_playerWet + (_delta * 4000000 * _scale)) min 1) max 0);
	};
	...
	if(_wetness == _absorbancy and !_isDrying)exitWith{
		_playerWet = (((_playerWet + (_delta * 4000000 * _scale)) min 1) max 0);
	};
...
diag_log "SERVER: END server\init.";